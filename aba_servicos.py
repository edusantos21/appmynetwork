# aba_servicos.py - PADRAO CORRIGIDO
...
class AbaServicos:
    ...
    def criar_aba(self):
        ...
        self.tree = ttk.Treeview(tree_frame, columns=("nome", "ip", "status", "latencia"), show="headings")
        ...
        self.tree.column("latencia", width=120)
    ...
    def atualizar_lista(self):
        ...
        for svc in self.servicos:
            ...
            latencia_raw = svc.get("latencia", 0)
            try:
            ...
            item_id = self.tree.insert("", "end", values=(
                svc.get("nome", ""), svc.get("ip", ""),
                status, latencia_str
            ))

            if nome_selecionado and nome_selecionado == svc.get("nome", ""):
                self.tree.selection_set(item_id)
                self.item_selecionado = item_id

    def _atualizar_linha(self, idx):
        svc = self.servicos[idx]
        nome = svc.get("nome", "")
        for item_id in self.tree.get_children():
            valores = self.tree.item(item_id, "values")
            if valores and valores[0] == nome:
                status = svc.get("status", "N/A")
                latencia_str = f"{int(svc.get('latencia', 0))}" if svc.get('latencia', 0) > 0 else "-"
                self.tree.item(item_id, values=(nome, svc.get("ip", ""), status, latencia_str))
                break

    def adicionar(self):
        dialog = ctk.CTkToplevel(self.parent)
        dialog.title("Adicionar Servico")
        dialog.geometry("400x250")
        dialog.minsize(350, 220)
        ...
        entry_nome = ctk.CTkEntry(main_frame, width=330)
        entry_nome.pack(anchor="w", pady=5)

        ctk.CTkLabel(main_frame, text="IP:").pack(anchor="w", pady=(10, 0))
        entry_ip = ctk.CTkEntry(main_frame, width=330)
        ...
        def salvar():
            ...
            servico = {
                "nome": nome, "ip": ip, "localidade": "",
                "tipo": "servico", "modo_operacao": "cliente",
            ...
            def salvar_db():
                try:
                    sucesso, svc_id = self.db.salvar_equipamento(servico)
                    if sucesso:
                        servico["id"] = svc_id
    ...
    def editar(self):
        if not self.item_selecionado:
            messagebox.showwarning("Aviso", "Selecione um servico")
            return
        ...
        nome_svc = valores[0]
        idx = next((i for i, s in enumerate(self.servicos) if s.get("nome") == nome_svc), None)
        if idx is None:
            return
        ...
        ctk.CTkLabel(main_frame, text="Nome:").pack(anchor="w", pady=(5, 0))
        entry_nome = ctk.CTkEntry(main_frame, width=330)
        entry_nome.insert(0, svc.get("nome", ""))
        entry_nome.pack(anchor="w", pady=5)

        ctk.CTkLabel(main_frame, text="IP:").pack(anchor="w", pady=(10, 0))
        entry_ip = ctk.CTkEntry(main_frame, width=330)
        entry_ip.insert(0, svc.get("ip", ""))
        entry_ip.pack(anchor="w", pady=5)
        ...
        def salvar():
            svc["nome"] = entry_nome.get().strip()
            svc["ip"] = entry_ip.get().strip()

            lbl_status.configure(text="Salvando...", text_color="orange")

            def salvar_db():
                try:
                    self.db.salvar_equipamento(svc)
                    self.monitor.atualizar_configuracoes(
                        self.db.listar_equipamentos(), self.config.get_configuracoes(),
                        self.db.listar_clientes()
                    )
    ...
    def excluir(self):
        ...
        if messagebox.askyesno("Confirmar", f"Excluir servico '{nome_svc}'?"):
            svc_id = self.servicos[idx].get("id")

            def excluir_db():
                try:
                    if svc_id:
                        self.db.excluir_equipamento(svc_id)
