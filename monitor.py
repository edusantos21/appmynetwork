# monitor.py - TCP ASYNCIO COM FALLBACK DE PORTAS (CORRIGIDO COM FIRMWARE)
import asyncio
import threading
import time
import socket
...
class Monitor:
    ...
    def parar(self):
        self.monitorando = False
        if self.loop_asyncio: self.loop_asyncio.call_soon_threadsafe(self.loop_asyncio.stop)
        print("Monitoramento parado")
    
    async def _ping_tcp_async(self, ip, porta_configurada=80):
        portas_para_tentar = []
        if porta_configurada not in portas_para_tentar: portas_para_tentar.append(porta_configurada)
        if 443 not in portas_para_tentar: portas_para_tentar.append(443)
        if 80 not in portas_para_tentar: portas_para_tentar.append(80)
        ...
        for porta in portas_para_tentar:
            try:
                ...
                try:
                    await asyncio.wait_for(asyncio.get_event_loop().sock_connect(sock, (ip, porta)), timeout=self.timeout_ping)
                    tempo = (time.perf_counter() - inicio) * 1000
                    sock.close()
                    return ip, True, round(tempo / 2, 1), porta
                except (asyncio.TimeoutError, ConnectionRefusedError, OSError): sock.close(); continue
    ...
    async def _monitorar_todos_async(self, ips):
        ...
        for ip in ips_unicos:
            ...
            for eq in self.equipamentos:
                if eq.get("ip") == ip and eq.get("porta"):
                    try: porta = int(eq.get("porta"))
                    except (ValueError, TypeError): porta = 80
                    break
        ...
        for ip, respondeu, latencia, porta_usada in resultados:
            if ip not in self.falhas: self.falhas[ip] = 0
            if respondeu: self.falhas[ip] = 0; self.ultima_latencia[ip] = latencia
            else: self.falhas[ip] = min(self.falhas[ip] + 1, self.max_falhas)
            online = self.falhas[ip] < self.max_falhas
            latencia_mostrar = latencia if respondeu else self.ultima_latencia.get(ip, 0)
            self.estado_dispositivos[ip] = {"online": online, "latencia": latencia_mostrar, "falhas": self.falhas[ip], "porta_usada": porta_usada if respondeu else None}
            chave = f"{ip}"
            estado_anterior = self.estados_anteriores.get(chave)
            novo_estado = "ONLINE" if online else "OFFLINE"
            if estado_anterior and estado_anterior != novo_estado: self._enviar_alerta(ip, novo_estado)
            self.estados_anteriores[chave] = novo_estado
    
    def _enviar_alerta(self, ip, novo_status):
        if not self.telegram_manager: return
        agora = time.time()
        if agora - self.ultima_verificacao_telegram > 30: self.telegram_configurado = self.telegram_manager.esta_configurado(); self.ultima_verificacao_telegram = agora
        if not self.telegram_configurado: return
        def enviar():
            try:
                hora_atual = datetime.now().strftime("%H:%M:%S")
                nome = ip; localidade = "-"
                for e in self.equipamentos:
                    if e.get("ip") == ip: nome = e.get("nome", ip); localidade = e.get("localidade", "-"); break
                for c in self.clientes:
                    if c.get("ip") == ip: nome = c.get("nome", ip); break
                if novo_status == "OFFLINE": msg = f"🔴 OFFLINE - {nome} | {localidade} | {ip}\n🕐 {hora_atual}"
                else:
        ...
        while self.monitorando:
            try:
                ...
                for e in self.equipamentos:
                    ...
                    if ip not in self.falhas: self.falhas[ip] = self.max_falhas
                for c in self.clientes:
                    ip = c.get("ip", "")
                    if ip: todos_ips.add(ip)
                    if ip not in self.falhas: self.falhas[ip] = self.max_falhas
                if todos_ips: self.loop_asyncio.run_until_complete(self._monitorar_todos_async(list(todos_ips)))
                ...
                time.sleep(max(0, self.intervalo - tempo_execucao))
            except Exception as e: print(f"Erro no loop: {e}"); time.sleep(5)
    ...
    def get_estado(self):
        ...
        for eq in self.equipamentos:
            ...
            if not isinstance(dados_snmp, dict): dados_snmp = {}
            resultado.append({
                "id": eq.get("id"),
                "nome": eq.get("nome", ""),
                "ip": ip,
                ...
                "ssh_usuario": eq.get("ssh_usuario", "ubnt"),
                "ssh_senha": eq.get("ssh_senha", ""),
                "ssh_porta": eq.get("ssh_porta", 22),
    ...
    def get_servidores_estado(self):
        resultado = []
        for eq in self.equipamentos:
            if eq.get('tipo') != 'servidor': continue
            ip = eq.get("ip", ""); estado = self.estado_dispositivos.get(ip, {})
            resultado.append({"id": eq.get("id"), "nome": eq.get("nome", ""), "ip": ip, "status": "ONLINE" if estado.get("online", False) else "OFFLINE", "latencia": estado.get("latencia", 0), "localidade": eq.get("localidade", ""), "porta": eq.get("porta", "80"), "falhas": estado.get("falhas", 0)})
        return resultado
    
    def get_energias_estado(self):
        resultado = []
        for eq in self.equipamentos:
            if eq.get('tipo') != 'energia': continue
            ip = eq.get("ip", ""); estado = self.estado_dispositivos.get(ip, {})
            resultado.append({"id": eq.get("id"), "nome": eq.get("nome", ""), "ip": ip, "status": "ONLINE" if estado.get("online", False) else "OFFLINE", "latencia": estado.get("latencia", 0), "localidade": eq.get("localidade", ""), "porta": eq.get("porta", "80"), "falhas": estado.get("falhas", 0)})
        return resultado
    ...
    def get_servicos_estado(self):
        resultado = []
        for eq in self.equipamentos:
            if eq.get('tipo') != 'servico': continue
            ip = eq.get("ip", ""); estado = self.estado_dispositivos.get(ip, {})
            resultado.append({"id": eq.get("id"), "nome": eq.get("nome", ""), "ip": ip, "status": "ONLINE" if estado.get("online", False) else "OFFLINE", "latencia": estado.get("latencia", 0), "localidade": eq.get("localidade", ""), "porta": eq.get("porta", "80"), "falhas": estado.get("falhas", 0)})
        return resultado
    ...
    def get_clientes_estado(self):
        ...
        for cli in self.clientes:
            ip = cli.get("ip", ""); estado = self.estado_dispositivos.get(ip, {})
            resultado.append({"id": cli.get("id"), "nome": cli.get("nome", ""), "ip": ip, "status": "ONLINE" if estado.get("online", False) else "OFFLINE", "latencia": estado.get("latencia", 0), "falhas": estado.get("falhas", 0)})
        return resultado
    
    def get_resumo(self):
        online = sum(1 for e in self.estado_dispositivos.values() if e.get("online"))
        offline = len(self.estado_dispositivos) - online
        return {"total": len(self.equipamentos) + len(self.clientes), "online": online, "offline": offline, "ultima_verificacao": self.ultima_verificacao.isoformat() if self.ultima_verificacao else None}
    
    def get_ultima_verificacao(self): return self.ultima_verificacao
    
    def testar_ping_unico(self, ip):
        async def testar(): return await self._ping_tcp_async(ip, 80)
        _, respondeu, latencia, _ = asyncio.run(testar())
        return (latencia, "ONLINE") if respondeu else (0, "OFFLINE")
