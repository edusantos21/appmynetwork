# monitor.py - TCP ASYNCIO COM FALLBACK DE PORTAS
import asyncio
import threading
import time
import socket
...
class Monitor:
        ...
        for porta in portas_para_tentar:
            try:
                ...
                try:
                    ...
                    # Divide por 3 para RTT real (2-way handshake)
                    latencia_real = round(tempo / 2, 1)
                    return ip, True, latencia_real, porta  # Retorna a porta que funcionou
    ...
    async def _monitorar_todos_async(self, ips):
        ...
        # Prepara tasks com portas personalizadas
        tasks = []
        for ip in ips_unicos:
            porta = 80  # padrão
            # Verifica se tem porta personalizada
        ...
        for ip, respondeu, latencia, porta_usada in resultados:
            ...
            # Usa última latência se falhou mas ainda está online
            ...
            self.estado_dispositivos[ip] = {
                ...
                "porta_usada": porta_usada if respondeu else None  # Opcional: mostra qual porta respondeu
            }
    ...
    def get_estado(self):
        """Retorna estado de todos os equipamentos"""
        resultado = []
        for eq in self.equipamentos:
            ip = eq.get("ip", "")
